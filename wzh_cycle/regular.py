    ...
    for key in product_time:
        ...
        for product in product_time[key]:
            ...
            if len(time_different)!=0 and len(buy_number)!=0:
                ret[key][product] = [sum(time_different)/len(time_different),sum(buy_number)/len(buy_number)]
    return ret
# in  
...
if __name__ == '__main__':
    ...
    ret_data = Predit(average_time,product_time,1503676800,1503936000,3600*24)
    finall = out_put(ret_data)
    print(finall.shape)
    col = range(2)
    row = range(len(finall))
    regular_data = pd.DataFrame(finall, index=row, columns=col)
    regular_data.to_csv('./data/regular_data.csv')
